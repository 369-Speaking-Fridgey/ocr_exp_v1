...
class TextRecognizer(object):
    ...
    def run(self, bbox, image):
        ...
        for idx, box in enumerate(text_box):
            ...
            draw.text((x1,y1),str(text),(0,0,255), font=font)
        return copy_image, answer_text

    def predict(self, image):
        if image.size == 0:
            return None
        image = cv2.resize(image, (Recog_CFG.IMG_W, Recog_CFG.IMG_H))
        aug = transforms.Compose([
            # transforms.Resize((Recog_CFG.IMG_H, Recog_CFG.IMG_W)),
            transforms.ToTensor(),
